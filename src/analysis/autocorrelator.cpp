...
*/
autocorrelator::autocorrelator(int pp, int mm, scalar deltaT)
    {
    p=pp;
    m=mm;
    minimumDistance = p/m;
...
void autocorrelator::initialize()
    ...
    vector<scalar> pZeroscalar(p,0.0);
...
void autocorrelator::growCorrelationLevel()
    ...
    vector<int> pZeroInt(p,0);
    vector<scalar> pZeroscalar(p,0.0);
    vector<scalar> pShiftscalar(p,-20000000000.0);
    shift.push_back(pShiftscalar);
    correlation.push_back(pZeroscalar);
...
*/
void autocorrelator::add(scalar w, int k)
    {
...
void autocorrelator::evaluate(bool normalize)
    ...
    for (int ii = 0; ii < p; ++ii)
        if(nCorrelation[0][ii] > 0)
            {
            scalar autocorr = correlation[0][ii]/nCorrelation[0][ii] - auxiliary;
            correlator.push_back(make_scalar2(ii*dt,autocorr));
            };
    ...
    for (int k = 1; k < nCorrelators; ++k)
        for (int ii = minimumDistance; ii < p; ++ii)
            if (nCorrelation[k][ii] > 0)
                {
                scalar autocorr = correlation[k][ii]/nCorrelation[k][ii] - auxiliary;
                scalar time = dt*ii*pow((scalar)m,k);
                correlator.push_back(make_scalar2(time,autocorr));
                };
    };
