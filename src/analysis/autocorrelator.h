...
class autocorrelator
    ...
    public:
        ...
        autocorrelator(int pp=16, int mm=2, scalar deltaT = 1.0);
        //! Add a new data point to the correlator
        void add(scalar w, int k=0);
        ...
        vector<scalar2> correlator;

        //! Set the time spacing
        void setDeltaT(scalar deltaT){dt=deltaT;};
    ...
    protected:
        ...
        vector<vector< scalar> > correlation;
        //!A vector of vector where incoming values get stored
        vector<vector< scalar> > shift;
    };
#endif
